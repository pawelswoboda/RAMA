...
std::tuple<dCOO, double, int> dual_update_cycle_length(const dCOO& A, const int cycle_length, const int num_dual_steps_per_cycle, const float tri_memory_factor, const float tol_ratio)
{
    ...
    std::tie(triangles_v1, triangles_v2, triangles_v3) = conflicted_cycles_cuda(A, cycle_length, tri_memory_factor, tol_ratio);
    ...
    return {A_repam, final_lb, triangles_v1.size()};
}

double dual_solver(dCOO& A, const int max_cycle_length, const int num_iter, const float tri_memory_factor, const int num_outer_itr, const float tol_ratio)
{
