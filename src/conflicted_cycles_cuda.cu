                                            ...
                                            int& v1_mid_edge_index, int& v2_mid_edge_index)
{
    while(v1_mid_edge_index < row_offsets[v1 + 1] && v2_mid_edge_index < row_offsets[v2 + 1])
    {
        ...
        const float v1_mid_edge_value = data[v1_mid_edge_index];
        const float v2_mid_edge_value = data[v2_mid_edge_index];
...
std::tuple<thrust::device_vector<int>, thrust::device_vector<int>, thrust::device_vector<int>> conflicted_cycles_cuda(const dCOO& A, const int max_cycle_length, int max_num_tri)
{
    ...
    if (num_positive_edges == 0 || num_rep_edges == 0 || max_cycle_length < 3)
